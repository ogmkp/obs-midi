...
void DeviceManager::Load(obs_data_t *data) {
  ...
  for (size_t i = 0; i < deviceCount; i++) {
    ...
    if (device->isEnabled()) {
      int portNumber = GetPortNumberByDeviceName(device->GetName());
      int outPort = GetOutPortNumberByDeviceName(device->GetOutName());
...
QStringList DeviceManager::GetOPL() { return opl; }
...
QVector<MidiHook *>
DeviceManager::GetMidiHooksByDeviceName(QString deviceName) {
...
/* Get this Device Manager state as OBS Data. (includes devices and their midi
 * hooks) This is needed to Serialize the state in the config.
...
void DeviceManager::broadcast(const RpcEvent &event) {
  ...
  // blog(1, "OBS EVENT %s -- %s",
  // event.updateType().toStdString().c_str(),obs_data_get_json(eventData));
  emit bcast(event.updateType(), QString(obs_data_get_json(eventData)));
};
