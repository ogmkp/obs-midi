...
obs_data_array_t *Utils::StringListToArray(char **strings, const char *key) {
  OBSDataArrayAutoRelease list = obs_data_array_create();
 ...
 * @property {Number} `cy`
 * @property {Number} `cx`
 * @property {Number} `alignment` The point on the source that the item is
 * manipulated from. The sum of 1=Left or 2=Right, and 4=Top or 8=Bottom, or
 * omit to center on that axis.
 * @property {String} `name` The name of this Scene Item.
 * @property {int} `id` Scene item ID
 * @property {Boolean} `render` Whether or not this Scene Item is set to
 * "visible".
 * @property {Boolean} `muted` Whether or not this Scene Item is muted.
 * @property {Boolean} `locked` Whether or not this Scene Item is locked and
 * can't be moved around
 * @property {Number} `source_cx`
 * @property {Number} `source_cy`
 * @property {String} `type` Source type. Value is one of the following:
 * "input", "filter", "transition", "scene" or "unknown"
 * @property {Number} `volume`
 * @property {Number} `x`
 * @property {Number} `y`
 * @property {String (optional)} `parentGroupName` Name of the item's parent (if
 * this item belongs to a group)
 * @property {Array<SceneItem> (optional)} `groupChildren` List of children (if
...
obs_sceneitem_t *Utils::GetSceneItemFromName(obs_scene_t *scene, QString name) {
  ...
  search.enumCallback = [](obs_scene_t *scene, obs_sceneitem_t *currentItem,
                           void *param) {
...
obs_sceneitem_t *
Utils::GetSceneItemFromRequestField(obs_scene_t *scene,
...
int Utils::GetTransitionDuration(obs_source_t *transition) {
  ...
  obs_data_item_t *transitionDurationItem =
      obs_data_item_byname(destinationSettings, "transition_duration");
  int duration =
      (transitionDurationItem ? obs_data_item_get_int(transitionDurationItem)
...
obs_data_t *Utils::GetTransitionData(obs_source_t *transition) {
  ...
  // When a transition starts and while it is running, SOURCE_A is the source
  // scene and SOURCE_B is the destination scene. Before the transition_end
  // event is triggered on a transition, the destination scene goes into
  // SOURCE_A and SOURCE_B becomes null. This means that, in transition_stop we
  // don't know what was the source scene
  ...
  if (item) {
    ...
    do {
      ...
      case OBS_DATA_STRING:
        query +=
            QUrl::toPercentEncoding(QString(obs_data_item_get_string(item)));
        break;
 ...
 * @property {int} `position.alignment` The point on the scene item that the
 * item is manipulated from.
 * @property {double} `rotation` The clockwise rotation of the scene item in
 * degrees around the point of alignment.
 * @property {double} `scale.x` The x-scale factor of the scene item.
 * @property {double} `scale.y` The y-scale factor of the scene item.
 * @property {int} `crop.top` The number of pixels cropped off the top of the
 * scene item before scaling.
 * @property {int} `crop.right` The number of pixels cropped off the right of
 * the scene item before scaling.
 * @property {int} `crop.bottom` The number of pixels cropped off the bottom of
 * the scene item before scaling.
 * @property {int} `crop.left` The number of pixels cropped off the left of the
 * scene item before scaling.
 * @property {bool} `visible` If the scene item is visible.
 * @property {bool} `locked` If the scene item is locked in position.
 * @property {String} `bounds.type` Type of bounding box. Can be
 ...
 * @property {double} `width` Scene item width (base source width multiplied by
 * the horizontal scaling factor)
 * @property {double} `height` Scene item height (base source height multiplied
 * by the vertical scaling factor)
 * @property {String (optional)} `parentGroupName` Name of the item's parent (if
 * this item belongs to a group)
 * @property {Array<SceneItemTransform> (optional)} `groupChildren` List of
 * children (if this item is a group)
 */
  ...
  obs_source_enum_filters(
      ...
      [](obs_source_t *parent, obs_source_t *child, void *param) {
        ...
        OBSDataAutoRelease filterData =
            Utils::GetSourceFilterInfo(child, enumParams->includeSettings);
