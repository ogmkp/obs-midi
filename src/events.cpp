...
events::events(DeviceManagerPtr srv)
    : _srv(srv), _streamStarttime(0), _lastBytesSent(0), _lastBytesSentTime(0),
      HeartbeatIsActive(false), pulse(false) {
  ...
  connect(&streamStatusTimer, SIGNAL(timeout()), this, SLOT(StreamStatus()));
  ...
  if (coreSignalHandler) {
    signal_handler_connect(coreSignalHandler, "source_create", OnSourceCreate,
                           this);
    signal_handler_connect(coreSignalHandler, "source_destroy", OnSourceDestroy,
                           this);
  }
}
...
void events::connectSourceSignals(obs_source_t *source) {
  ...
  signal_handler_connect(sh, "reorder_filters", OnSourceFilterOrderChanged,
                         this);
  ...
  if (sourceType == OBS_SOURCE_TYPE_SCENE) {
    ...
    signal_handler_connect(sh, "item_visible", OnSceneItemVisibilityChanged,
                           this);
...
void events::disconnectSourceSignals(obs_source_t *source) {
  ...
  signal_handler_disconnect(sh, "audio_sync", OnSourceAudioSyncOffsetChanged,
                            this);
  signal_handler_disconnect(sh, "audio_mixers", OnSourceAudioMixersChanged,
                            this);
  ...
  signal_handler_disconnect(sh, "reorder_filters", OnSourceFilterOrderChanged,
                            this);
  ...
  signal_handler_disconnect(sh, "item_visible", OnSceneItemVisibilityChanged,
                            this);
  ...
  signal_handler_disconnect(sh, "transition_video_stop", OnTransitionVideoEnd,
                            this);
}
...
void events::hookTransitionPlaybackEvents() {
  ...
  for (uint i = 0; i < transitions.sources.num; i++) {
    ...
    signal_handler_disconnect(sh, "transition_video_stop", OnTransitionVideoEnd,
                              this);
    signal_handler_connect(sh, "transition_video_stop", OnTransitionVideoEnd,
                           this);
  }
 ...
 * @return {Array<SceneItem>} `sources` List of scene items in the new scene.
 ...
 * @since 0.3
 */
void events::OnSceneListChange() { broadcastUpdate("ScenesChanged"); }
 ...
 * @since 4.0.0
 */
void events::OnProfileChange() { broadcastUpdate("ProfileChanged"); }
 ...
 * @since 4.0.0
 */
void events::OnProfileListChange() { broadcastUpdate("ProfileListChanged"); }
 ...
 * @since 0.3
 */
void events::OnRecordingStarting() { broadcastUpdate("RecordingStarting"); }
 ...
 * @since 0.3
 */
void events::OnRecordingStarted() { broadcastUpdate("RecordingStarted"); }
 ...
 * @since 0.3
 */
void events::OnRecordingStopping() { broadcastUpdate("RecordingStopping"); }
 ...
 * @since 0.3
 */
void events::OnRecordingStopped() { broadcastUpdate("RecordingStopped"); }
 ...
 * @since 4.7.0
 */
void events::OnRecordingPaused() { broadcastUpdate("RecordingPaused"); }
 ...
 * @since 4.7.0
 */
void events::OnRecordingResumed() { broadcastUpdate("RecordingResumed"); }
 ...
 * @since 4.2.0
 */
void events::OnReplayStarting() { broadcastUpdate("ReplayStarting"); }
 ...
 * @since 4.2.0
 */
void events::OnReplayStarted() { broadcastUpdate("ReplayStarted"); }
 ...
 * @since 4.2.0
 */
void events::OnReplayStopping() { broadcastUpdate("ReplayStopping"); }
 ...
 * @since 4.2.0
 */
void events::OnReplayStopped() { broadcastUpdate("ReplayStopped"); }
 ...
 * @since 0.3
 */
void events::OnExit() { broadcastUpdate("Exiting"); }
 ...
 * started.
 * @return {int} `num-total-frames` Total number of frames transmitted since the
 * stream started.
 ...
 * @return {int} `render-missed-frames` Number of frames missed due to rendering
 * lag
 * @return {int} `output-total-frames` Number of frames outputted
 * @return {int} `output-skipped-frames` Number of frames skipped due to
 * encoding lag
 ...
 *
 * @return {boolean} `pulse` Toggles between every JSON message as an "I am
 * alive" indicator.
 ...
 * @return {String} `sourceType` Source type. Can be "input", "scene",
 ...
 *
 * @return {String} `sourceName` Source name
 * @return {Array<Object>} `mixers` Routing status of the source for each audio
 * mixer (array of 6 values)
 * @return {int} `mixers.*.id` Mixer number
 * @return {boolean} `mixers.*.enabled` Routing status
 * @return {String} `hexMixersValue` Raw mixer flags (little-endian, one bit per
 * mixer) as an hexadecimal value
 *
 * @api events
 ...
 *
 * @return {String} `scene-name` Name of the scene where items have been
 * reordered.
 ...
 * @return {Array<SceneItem>} `sources` List of sources composing the scene.
 * Same specification as [`GetCurrentScene`](#getcurrentscene).
 *
 * @api events
 ...
 * @property {int} `render-missed-frames` Number of frames missed due to
 * rendering lag
 * @property {int} `output-total-frames` Number of frames outputted
 * @property {int} `output-skipped-frames` Number of frames skipped due to
 * encoding lag
 * @property {double} `average-frame-time` Average frame render time (in
 * milliseconds)
